...
export class DataUploadFileComponent implements OnInit {
  ...
  readonly maxSize = 15485760; //10MB => 10 * 2 ** 20
  formValues: DataModel[];
  uploadForm: FormGroup;

  onDoneClicked = () => {
    let algorithmIndex = this.algorithms.findIndex(
      (algorithm) =>
        algorithm.name === this.uploadForm.get("algorithmName").value
    );
    ...
    this.trainFileSizePercentage = Math.round(
      (trainFile.size * 100) / totalFileSize
    );
    this.testFileSizePercentage = Math.round(
      (testFile.size * 100) / totalFileSize
    );
    this.trainFileSizeRatio =
      100 / (this.trainFileSizePercentage ? this.trainFileSizePercentage : 1);
    this.testFileSizeRatio =
      100 / (this.testFileSizePercentage ? this.testFileSizePercentage : 1);

    this.formValues = [
      {
        value: "Conjunto de datos",
        viewValue: this.uploadForm.get("dataName").value,
      },
      { value: "Archivo de entrenamiento", viewValue: trainFile.name },
      { value: "Archivo de prueba", viewValue: testFile.name },
      {
        value: "Algoritmo",
        viewValue: this.algorithms[algorithmIndex].description,
      },
    ];
  };
  ...
  onSubmit = () => {
    ...
    formData.append(
      "trainFile",
      this.uploadForm.get("trainFile").value.files[0]
    );
    formData.append("testFile", this.uploadForm.get("testFile").value.files[0]);
    formData.append(
      "algorithmName",
      this.uploadForm.get("algorithmName").value
    );
      ...
      .pipe(
        this.utils.uploadProgress((percentDone) => {
          if (percentDone < this.trainFileSizePercentage) {
            this.trainFileUploadProgress = Math.round(
              percentDone * this.trainFileSizeRatio
            );
          } else {
            this.trainFileUploadProgress = 100;
            this.testFileUploadProgress = Math.round(
              (percentDone - this.trainFileSizePercentage) *
                this.testFileSizeRatio
            );
          }
        }),
  ...
  onBackendResponse(response: any): void {
    this.backend.getResults(response.taskstatusurl).subscribe({
      next: (data: any) => {
